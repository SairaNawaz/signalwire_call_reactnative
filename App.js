...
import {TouchableOpacity, StyleSheet, Text, View, TextInput, KeyboardAvoidingView, Alert, Button} from 'react-native';
...
export default class App extends Component<Props> {
    ...
    async getJWTToken() {
        console.log("Inside getting token");

        axios.post(`https://....`
            ,
            {
                "resource": this.state.inbox,
                "expires_in": 120
            }
            , {
                auth: {
            ...
            if (response.data && response.data.jwt_token) {
                this.initClient(response.data.jwt_token);
            } else {
    ...
    async initClient(jwtToken) {
        ...
        this.client.subscribe({
            ...
            channels: [this.state.inbox, this.state.inbox2]
        }).then(r => {
            console.log(r);
        })
    ...
    async makeCall() {
        await this.client.newCall({
            destinationNumber: this.state.destination,
    ...
    _statusinbox() {
        return this.state.connected ? this.state.inbox + ' Online' : this.state.inbox + ' Offline'
    }
        ...
        if (this.state.call) {
            ...
            return (
                <View style={styles.wrapperMiddle}>
                    {streamURL && <RTCView mirror={false} objectFit='contain' streamURL={streamURL}
                                           style={{width: '100%', height: '100%'}} zOrder={1}/>}
                </View>
            )
        } else {
            return (
                <View style={styles.wrapperMiddle}>
                    <Text style={[styles.instructions, {marginTop: 30}]}>Enter destination number:</Text>
                    <TextInput
        ...
        return (
            <KeyboardAvoidingView style={styles.container}>
                <View style={styles.wrapperTop}>
                    ...
                    <Text style={styles.instructions}>Status: {this._statusinbox()}</Text>
                    <Text style={styles.instructions}>Set Online? Connect to SignalWire</Text>
                    <Button title='Connect SignalWire' onPress={() => {
                        console.log("Connecting Signalwire...");
                        this.getJWTToken().then(r => console.log("Connecting inbox Signalwire2..."));
                    }}/>
                </View>
                {this._middle()}
                {this._bottom()}
